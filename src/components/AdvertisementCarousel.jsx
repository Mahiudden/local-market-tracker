...
const AdvertisementCarousel = () => {
  ...
  return (
    <section className="my-6 flex flex-col items-center justify-center">
      <h3 className="text-xl font-bold text-blue-700 mb-4 flex items-center gap-2"><FaBullhorn className="text-yellow-400" /> বিজ্ঞাপন ও অফার</h3>
      <div className="relative w-full max-w-xs sm:max-w-md md:max-w-lg">
        <AnimatePresence mode="wait" initial={false}>
          <motion.div
            ...
            animate="animate"
            exit="exit"
            className="bg-white/80 rounded-2xl shadow-xl p-6 min-w-[250px] flex flex-col items-center transition hover:shadow-2xl border-2 border-blue-100 absolute w-full"
            style={{ position: 'absolute' }}
          >
            <img src={ad.image} alt={ad.title} className="w-full h-[200px] object-cover rounded mb-3 shadow" />
            ...
            <Link to={ad.link ? ad.link : '/offers'}  className="text-blue-600 hover:underline font-semibold mb-2">বিস্তারিত</Link>
          </motion.div>
        </AnimatePresence>
      ...
      <div className="flex gap-2 mt-86">
        {ads.map((_, i) => (
          <motion.button
            key={i}
            onClick={() => setIndex(i)}
            className={`w-3 h-3 rounded-full border-2 border-blue-300 transition-all duration-200 ${i === index ? 'bg-blue-500 border-blue-500 scale-125' : 'bg-white'}`}
