    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-4 sm:p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
        ...
        <div>
          ...
          <input type="number" value={pricePerUnit} onChange={e => setPricePerUnit(e.target.value)} placeholder='কমপক্ষে 7০' className="w-full border rounded px-3 py-2" required />
        </div>
        <div className="md:col-span-2">
...
function MyProductsTable({ vendorUid }) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (Number(form.pricePerUnit) < 70) {
      await Swal.fire({
        icon: 'error',
        title: 'ত্রুটি!',
        text: 'প্রোডাক্টের দাম ৭০ টাকার নিচে হতে পারবে না।',
    ...
    try {
      ...
      await Swal.fire({
        icon: 'success',
        title: 'আপডেট সফল!',
        text: 'প্রোডাক্ট সফলভাবে আপডেট হয়েছে।',
        confirmButtonText: 'ঠিক আছে'
      });
      fetchProducts();
      closeEditModal();
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      ...
      text: 'আপনি কি নিশ্চিতভাবে এই প্রোডাক্টটি ডিলিট করতে চান?',
      icon: 'warning',
    ...
    try {
      ...
      await Swal.fire({
        icon: 'success',
        title: 'ডিলিট সফল!',
        text: 'প্রোডাক্ট সফলভাবে ডিলিট হয়েছে।',
        confirmButtonText: 'ঠিক আছে'
      });
      fetchProducts();
  ...
  return (
    <div className="bg-white rounded-xl shadow p-4 sm:p-6 mt-8">
                  ...
                  <td className="px-2 sm:px-4 py-2 border">
                    ...
                    {product.status === 'rejected' && (
                      <div className="flex flex-col items-center">
                        ...
                        {product.reason && (
                          <FeedbackBox>রিজেক্টের কারণ: {product.reason}</FeedbackBox>
                        )}
                        {product.feedback && (
                          <FeedbackBox>ফিডব্যাক: {product.feedback}</FeedbackBox>
                        )}
                      </div>
                    )}
                  </td>
      ...
      {/* SweetAlert2 দিয়ে কনফার্মেশন হবে, আলাদা modal আর দেখাবো না */}
    </div>
  );
}
...
function MyAdsTable() {
  ...
  useEffect(() => {
    ...
    getAllAds()
      .then(res => {
        // শুধু নিজের বিজ্ঞাপন দেখাবে
        const filtered = (res.data || []).filter(ad => ad.vendorUid === user?.uid);
        setAds(filtered);
        setLoading(false);
      })
      .catch(() => {
        setError('বিজ্ঞাপন লোড করা যায়নি');
        setLoading(false);
      });
  }, [user]);
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: 'আপনি কি নিশ্চিত?',
      text: 'আপনি কি নিশ্চিতভাবে এই বিজ্ঞাপনটি ডিলিট করতে চান?',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
  ...
  return (
    <div className="bg-white rounded-xl shadow p-4 sm:p-6">
      ...
      {loading ? (
        ...
        <div className="text-gray-500">কোনো বিজ্ঞাপন নেই</div>
      ) : (
        <div className="overflow-x-auto">
          {/* Responsive Table */}
...
// Helper: ফিডব্যাক টেক্সটের জন্য স্টাইল
