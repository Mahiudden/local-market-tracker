...
const Login = () => {
  ...
  const handleReset = async (e) => {
    ...
    try {
      ...
      setResetMsg("পাসওয়ার্ড রিসেট লিঙ্ক আপনার ইমেইলে পাঠানো হয়েছে!");
    } catch (err) {
      setResetMsg("রিসেট করতে সমস্যা হয়েছে: " + err.message);
    }
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 to-green-100 py-8">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md space-y-5 border border-blue-100">
        ...
        <span className="text-gray-600">একাউন্ট নেই?</span>
        <button className="text-blue-600 hover:underline ml-2" onClick={() => navigate('/register')}>রেজিস্টার করুন</button>
      </div>
        <button
          type="button"
